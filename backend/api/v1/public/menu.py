...
from crud import menu_crud, user_crud, activity_crud
from models.enums import AccessLevel, ActivityType
from schemas.public.menu import (
...
@router.get("/{id}/content", response_model=MenuContentResponse, status_code=status.HTTP_200_OK)
async def get_menu_item_content(
    ...
    # Проверяем доступ
    access_level = AccessLevel.PREMIUM if user.subscription_type == "premium" else AccessLevel.FREE
    if menu_item.access_level == AccessLevel.PREMIUM and access_level != AccessLevel.PREMIUM:

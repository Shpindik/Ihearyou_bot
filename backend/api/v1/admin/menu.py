...
from schemas.admin.menu import (
...
@router.get("/", response_model=AdminMenuItemListResponse, status_code=status.HTTP_200_OK)
async def get_admin_menu_items(
    ...
    access_level: str = Query(None, description="Фильтр по уровню доступа (free, premium)"),
...
@router.put("/{id}", response_model=AdminMenuItemResponse, status_code=status.HTTP_200_OK)
async def update_menu_item(
    id: int,
    request: AdminMenuItemUpdate,
    token: str = Depends(security)
) -> AdminMenuItemResponse:
    """
...
@router.get("/{id}/content-files", response_model=list[AdminContentFileResponse], status_code=status.HTTP_200_OK)
...
@router.post("/{id}/content-files", response_model=AdminContentFileResponse, status_code=status.HTTP_201_CREATED)
async def create_content_file(
    id: int,
    request: AdminContentFileCreate,
    token: str = Depends(security)
) -> AdminContentFileResponse:
    """
...
@router.put("/content-files/{file_id}", response_model=AdminContentFileResponse, status_code=status.HTTP_200_OK)
async def update_content_file(
    file_id: int,
    request: AdminContentFileUpdate,
    token: str = Depends(security)
) -> AdminContentFileResponse:
    """

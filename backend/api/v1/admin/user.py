...
from backend.schemas.admin.user import (
...
@router.get(
    "/", response_model=AdminTelegramUserListResponse, status_code=status.HTTP_200_OK
)
async def get_telegram_users(
    ...
    subscription_type: str = Query(
        None, description="Фильтр по типу подписки (free, premium)"
    ),
...
@router.get(
    "/{id}", response_model=AdminTelegramUserResponse, status_code=status.HTTP_200_OK
)

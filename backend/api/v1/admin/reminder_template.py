...
router = APIRouter(
    prefix="/admin/reminder-templates", tags=["Admin Reminder Templates"]
)
security = HTTPBearer()
...
async def create_reminder_template(
    request: AdminReminderTemplateCreate, token: str = Depends(security)
) -> AdminReminderTemplateResponse:
...
async def update_reminder_template(
    id: int, request: AdminReminderTemplateUpdate, token: str = Depends(security)
) -> AdminReminderTemplateResponse:
...
async def delete_reminder_template(id: int, token: str = Depends(security)) -> None:
